...
# details  : 
# example :
compute_weight0 <- function(conf,rho){
  n <- ncol(conf)
  E <- nrow(conf)
  ESS <- 0
  ESS_k <- rep(0,E)
...
compute_weight <- function(conf,rho,n_y){
  E <- length(conf)
  ESS <- 0
  ESS_k <- rep(0,E)
  weights <- list()
  ...
  for(k in 1:E){
    rho_k_sq[k] <- sum( rho[k,-k]^2)
    ESS_k[k]    <- sum(conf[[k]]) / (1 + rho_k_sq[k]) *
      (n_y / (length(conf[[k]]) * E))                                           # renormalise pour que l'ensemble des experts compte au maximum autant que les donnees
    if( sum(conf[[k]]) == 0 ){
      weights[[k]] <- rep(0,length(conf[[k]]))
    }else{
      weights[[k]] <- conf[[k]] * ESS_k[k] / sum(conf[[k]])
    }
  }
