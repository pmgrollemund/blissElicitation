...
# R script
sourceCpp("./Bliss_eli_multiple_rcpp_MWG.cpp")
source("./Bliss_eli_algorithms_MWG.R")
...
#                 y | \mu, \beta, \sigma, m, l.
#              beta_s_expert : a List containing for all covariable, the 
#                 elicited signed function beta (as a vector).
#              MH_proposal : a string character, indicating how to propose a 
#                 value for beta_tilde using a Metropolis step.
...
Bliss_multiple   <- function(data,param,plot=FALSE){
  ...
  for(j in 1:n_chains){
    ...
    chains[[j]]$res.Gibbs_Sampler <- Bliss_MWG_multiple(data,param)
    
    # head(chains[[j]]$res.Gibbs_Sampler$trace)
    # head(chains[[j]]$res.Gibbs_Sampler$trace[sum(chains[[j]]$res.Gibbs_Sampler$trace) >0,])
    # 
    # which(is.na(chains[[j]]$res.Gibbs_Sampler$trace))
    # chains[[j]]$res.Gibbs_Sampler$trace[266,]
    # chains[[j]]$res.Gibbs_Sampler$trace[3e3,]
    # sum(chains[[j]]$res.Gibbs_Sampler$trace[1:267,13])
    ...
    chains[[j]]$beta_s <- compute_beta_s_mult(chains[[j]]$beta_functions)
